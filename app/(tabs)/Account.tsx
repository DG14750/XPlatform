...
export default function Account() {
  ...
  const onSavePressed = async () => {
    ...
    try {
      ...
      await saveUserPlatforms(
        user.uid,
        tempOwnedPlatforms,
        ownedPlatforms.map((p) => p.platform)
      );
      setOwnedPlatforms(
        tempOwnedPlatforms.map((platform) => ({
  ...
  return (
    <View style={styles.screen}>
      <AppHeader title="Account" onSignOut={handleSignOut} />
      ...
      >
        <View style={Boxes.formBox2}>
          ...
          {display && (
            <Text style={[Typography.body, Boxes.textImputBox]}>
              {userName}
            </Text>
          )}
          {edit && (
            ...
            <View style={styles.iconText}>
              {ownedPlatforms.map((ownedPlatform) => (
                <View style={styles.option} key={ownedPlatform.platform}>
                  <Text style={[styles.metaSmall, styles.chip]}>
                  ...
                  <View style={styles.option} key={platform}>
                    <Pressable
                      style={[
                        Boxes.checkBox,
                        {
                          backgroundColor: tempOwnedPlatforms.includes(platform)
                            ? "#3997fbff"
                            : "#000000",
                        },
                      ]}
        ...
        <View style={Boxes.formBox}>
          <View style={styles.iconButton}>
            ...
            <Pressable onPress={handleSignOut}>
              <Text style={[Typography.subtitle, { color: "red" }]}>
                Logout
              </Text>
            </Pressable>
          </View>
